package com.pro12.myssm.basedao;
    ...
    private void setValue(Object obj ,  String property , Object propertyValue) throws NoSuchFieldException, IllegalAccessException, ClassNotFoundException, NoSuchMethodException, InvocationTargetException, InstantiationException {
        ...
        if(field!=null){
            ...
            if (isMyType(typeName)) {
                ...
                Constructor constructor = typeNameClass.getDeclaredConstructor(java.lang.Long.class);
    ...
    protected T load(String sql , Object... params){
        ...
        try{
            ...
            if(rs.next()){
                ...
                for(int i = 0 ; i<columnCount;i++){
                    String columnLabelName = rsmd.getColumnLabel(i+1);            //fid   fname   price
                    Object columnValue = rs.getObject(i+1);     //33    苹果      5
                    if ("id".equals(columnLabelName)) {
                        columnValue = Long.parseLong(String.valueOf(columnValue));
                    } else if ("topicDate".equals(columnLabelName)
                            || "replyDate".equals(columnLabelName)
                            || "hostReplyDate".equals(columnLabelName)
                            || "birth".equals(columnLabelName))
                    {
                        columnValue = Timestamp.valueOf((LocalDateTime) columnValue);
                    }
                    setValue(entity,columnLabelName,columnValue);
                }
                return entity ;
            }
        }catch (Exception e){
        ...
        try{
            ...
            while(rs.next()){
                ...
                for(int i = 1 ; i<=columnCount;i++){
                    String columnLabelName = rsmd.getColumnLabel(i);            //fid   fname   price
                    Object columnValue = rs.getObject(i);     //33    苹果      5
                    if ("id".equals(columnLabelName)) {
                        columnValue = Long.parseLong(String.valueOf(columnValue));
                    } else if ("topicDate".equals(columnLabelName)
                                || "replyDate".equals(columnLabelName)
